...
def parse_buffer(buffer):
    ...
    if len(buffer) < 9:
        ...
        return None
    try:
        power = struct.unpack_from('<h', buffer, 6)[0]  # Confirm this is correct (16-bit signed integer at offset 6)
        cadence = struct.unpack_from('<H', buffer, 4)[
                      0] / 2  # Confirm this is correct (16-bit unsigned integer at offset 4)
        speed = struct.unpack_from('<H', buffer, 2)[
                    0] / 100  # Confirm this is correct (16-bit unsigned integer at offset 2)
        heart_rate = struct.unpack_from('<b', buffer, 8)[
            0]  # Confirm this is correct (8-bit signed integer at offset 8)
        print(f'Parsed values -> Power: {power}, Cadence: {cadence}, Speed: {speed}, Heart Rate: {heart_rate}')
